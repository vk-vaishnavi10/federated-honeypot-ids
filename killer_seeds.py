"""Killer experiment across 5 seeds -> mean ± std. Q1 robustness requirement."""
...
def run(seed):
    ...
    solo=rr(train_ids(IDS(),hps[0][0],hps[0][1],ep=80))
    g=IDS()
    for _ in range(10):
        lms,sz=[],[]
    ...
    fed=rr(g)
    g=IDS(); G=train_gan(np.vstack([r for (_,_,r) in hps]))
...
res={"solo":[],"fed":[],"gan":[]}
for s in seeds:
    a,b,c=run(s); res["solo"].append(a); res["fed"].append(b); res["gan"].append(c)
    print(f"seed {s}: solo={a:.3f} fed={b:.3f} gan={c:.3f}")

print(f"\n{'setup':34s} mean ± std   (n=5 seeds)")
print("-"*56)
for k,name in [("solo","1. single honeypot (isolated)"),
               ("fed","2. federated, no GAN"),
               ("gan","3. federated + GAN (ours)")]:
    a=np.array(res[k]); print(f"{name:34s} {a.mean():.3f} ± {a.std():.3f}")
